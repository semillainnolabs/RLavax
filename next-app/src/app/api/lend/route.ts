...
const aaveAbi = [{ name: "supply", type: "function", stateMutability: "nonpayable", inputs: [{ name: "asset", type: "address" }, { name: "amount", type: "uint256" }, { name: "onBehalfOf", type: "address" }, { name: "referralCode", type: "uint16" }], outputs: [] }] as const;
...
export async function POST(req: Request) {
  try {
    ...
    console.log("--- LEND (AAVE) ---", { walletId, cleanAddress: addr, amount });

    const approveData = encodeFunctionData({ abi: erc20Abi, functionName: "approve", args: [AAVE_POOL as `0x${string}`, maxUint256] });
    ...
    const depositData = encodeFunctionData({ abi: aaveAbi, functionName: "supply", args: [USDC, parsedAmount, addr, 0] });
    const depositTx = await privyRpc(walletId, "eip155:84532", { to: AAVE_POOL, data: depositData, chain_id: 84532 });
